...
const TicketDetailModal: React.FC<TicketDetailModalProps> = ({
  ...
  const handleGenerateSmartReply = async () => {
    ...
    try {
      const smartResponse = await responseAgentService.generateSmartResponse(ticket);
      
      // Create a new AI suggestion from the smart response
      const newSuggestion: AIResponse = {
      ...
      // Add the new suggestion to the top of the list
      setAiSuggestions([newSuggestion, ...aiSuggestions]);
      
      // Auto-select the generated suggestion
      handleUseSuggestion(newSuggestion);
      
      // Update status if suggested
      if (smartResponse.suggested_status && smartResponse.suggested_status !== ticket.status) {
        setNewStatus(smartResponse.suggested_status);
      }
      
      toast({
        title: "Smart Reply Generated",
        description: "AI has crafted a personalized response based on similar tickets",
      });
    } catch (error) {
                    ...
                    <div 
                      ...
                      className={`p-4 border rounded-lg space-y-3 ${
                        selectedSuggestion === suggestion.id 
                          ...
                          : suggestion.id.startsWith('smart-reply') 
